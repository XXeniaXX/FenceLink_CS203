...
const ManagePlayerPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchPlayers = async () => {
            try {
                const response = await fetch('http://47.129.36.1:8080/api/players/all');
                if (response.ok) {
    ...
    const handleDelete = async (player) => {
        ...
        try {
            ...
            if (player.user && player.user.id) {
                ...
                response = await fetch(`http://47.129.36.1:8080/api/users/${player.user.id}`, {
                    method: 'DELETE',
                    headers: {
                ...
                response = await fetch(`http://47.129.36.1:8080/api/players/${player.id}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
